...
namespace DataIngestion.Ingest.Services
{
	public class PersistDataService : IPersistDataService
	{
		...
		public bool PersistData2()
		{
			try
			{
				...
				var pkgLocation = Path.Combine(Directory.GetCurrentDirectory(),
					@"..\..\..\..\ImportToSql\\obj\\Development\\Package.dtsx");
		...
		public bool PersistData1()
		{
			try
			{
				...
				var destPath = _configuration.GetValue<string>("UnZip:ExtractToPathPath");
		...
		private Artist GetArtistRow(string row)
		{
			try
			{
				...
				var artist = new Artist
				{
					...
					ArtistTypeId = strings.ElementAtOrDefault(4) != null ? strings[5] : string.Empty
				};
	...
	public class MyEventListener : IDTSEvents100
	{
		...
		public void OnWarning(IDTSRuntimeObject100 pSource, int WarningCode, string SubComponent, string Description,
			string HelpFile,
			int HelpContext, string IDOfInterfaceWithError)
		{
		}

		public void OnInformation(IDTSRuntimeObject100 pSource, int InformationCode, string SubComponent,
			string Description,
			string HelpFile, int HelpContext, string IDOfInterfaceWithError, ref bool pbFireAgain)
		{
		}

		public void OnError(IDTSRuntimeObject100 pSource, int ErrorCode, string SubComponent, string Description,
			string HelpFile,
		...
		public void OnProgress(IDTSTaskHost100 pTaskHost, string ProgressDescription, int PercentComplete,
		...
		public void OnCustomEvent(IDTSTaskHost100 pTaskHost, string EventName, string EventText,
