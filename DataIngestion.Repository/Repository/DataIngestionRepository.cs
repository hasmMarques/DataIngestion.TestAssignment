	...
	public class DataIngestionRepository : IDataIngestionRepository
	{
		...
		public async Task<bool> InsertCollection(Collection collection)
		{
			try
			{
				...
				await using var entities = new DataIngestionContext();
				if (await entities.Collection.AnyAsync(o => o.CollectionId == collection.CollectionId)
		...
		public async Task<List<AlbumArtist>> GetArtists(long artistId)
		{
			try
			{
				...
				var artists = (from at in entities.Artist
					...
					select new AlbumArtist {AlbumArtistId = at.ArtistId, Name = at.Name}).Distinct().ToList();
